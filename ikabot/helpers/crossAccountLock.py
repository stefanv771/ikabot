...
def _queue_file_path():
    ...
    """
    return os.path.join(os.path.expanduser("~"), QUEUE_FILE_NAME)
...
def _queue_mutex_path():
    ...
    """
    return os.path.join(os.path.expanduser("~"), QUEUE_MUTEX_NAME)
...
def _acquire_queue_mutex(timeout=MUTEX_TIMEOUT_SECONDS):
    ...
    while time.time() - start_time < timeout:
        try:
            ...
            # someone else holds the mutex right now - but if it's been
            # held suspiciously long (a process died mid-update without
            # releasing it), steal it rather than deadlocking forever
            try:
...
def _prune_stale_tickets(tickets):
    ...
    """
    now = time.time()
    return [t for t in tickets if now - t.get("timestamp", 0) < STALE_LOCK_SECONDS]
...
def _remove_stale_lock_if_any(lock_file):
    ...
    present, meaning it's still meant to be actively held - it's older
    than STALE_LOCK_SECONDS (the owner most likely crashed outright).
    """
    try:
        ...
        if available_at is not None:
            if now >= available_at:
                os.remove(lock_file)
        elif now - lock_data.get("timestamp", 0) > STALE_LOCK_SECONDS:
            os.remove(lock_file)
    except Exception:
